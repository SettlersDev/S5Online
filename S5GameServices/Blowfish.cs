...
namespace S5GameServices
{
    ...
    public class Blowfish
    {
        ...
        public void EncipherPadded(ref byte[] data)
        {
            var len = data.Length;
            var paddedLen = (int)(Math.Ceiling(len / 8.0) * 8);
            Array.Resize(ref data, paddedLen + 2);
            Encipher(data, paddedLen);
            data[paddedLen] = (byte)len;
            data[paddedLen + 1] = (byte)(len >> 8);
        }

        public void EncipherPadded(byte[] data, ref int len)
        {
            var paddedLen = (int)(Math.Ceiling(len / 8.0) * 8);
            //Array.Resize(ref data, paddedLen + 2);
            if (data.Length < paddedLen)
                throw new Exception("Array too small to hold encrypted data!");
            Encipher(data, paddedLen);
            data[paddedLen] = (byte)len;
            data[paddedLen + 1] = (byte)(len >> 8);
            len = paddedLen;
        ...
        public void DecipherPadded(byte[] data, ref int length)
        {
            int dataLen = (data[length - 1] << 8) + (data[length - 2]);
            Decipher(data, length - 2);
            length = dataLen;
        }

        public void DecipherPadded(ref byte[] data)
        {
            int dataLen = (data[data.Length - 1] << 8) + (data[data.Length - 2]);
            Decipher(data, data.Length - 2);
